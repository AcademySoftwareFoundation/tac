...
For projects without a website, the primary GitHub repo ```README.md``` should have this information.
...
The recommendation is to have this specifically outlined in the ```README.md``` or in a separate ```CONTRIBUTING.md``` file.
...
Generally, all projects at the ASWF are under an OSI Approved license.

### The project MUST post the license(s) of its results in a standard location in their source repository.

It's recommended to have this in a ```LICENSE``` file in each repository.
